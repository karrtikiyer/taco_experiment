"""
Vendored from FlagOpen/TACO: https://github.com/FlagOpen/TACO/blob/main/metrics/testing_util.py
Modified from https://github.com/hendrycks/apps/blob/main/eval/testing_util.py

Handles both call-based and stdin-based code execution for TACO problems.
"""
...
EXECUTION_RESULTS = {
    1: "passed", 0: "false", -1: "timeout",
    -2: "runtime_error", -3: "returncode:{code}", -4: "compile_error"
}
...
MAX_EXECUTION_MEMORY_BYTES = 512 * 1024 * 1024  # 512 MB


def _set_memory_limit():
    """preexec_fn for subprocess.run to cap child memory (best-effort).

    On macOS, setrlimit may fail for certain limit types; we catch and
    continue since this is a defense-in-depth measure.
    """
    try:
        import resource
        for limit in (resource.RLIMIT_DATA, resource.RLIMIT_AS):
            try:
                resource.setrlimit(limit, (MAX_EXECUTION_MEMORY_BYTES, MAX_EXECUTION_MEMORY_BYTES))
...
def execute_cb_code(method, inputs_list, outputs_list, timeout, early_stop=False, debug=False):
    reliability_guard(maximum_memory_bytes=MAX_EXECUTION_MEMORY_BYTES)
    results = []
    debug_infos = {}
    for index, inputs in enumerate(inputs_list):
        ...
        if early_stop and results and results[-1][0] is False:
            break
...
def execute_std_code(synthesized_code, inputs_list, outputs_list, timeout, early_stop=False, debug=False):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        temp_program_path = _write_temp_file(synthesized_code, tmpdir, "program.py")
        if debug:
        ...
        for i, inputs in enumerate(inputs_list):
            ...
            if isinstance(outputs, list):
                outputs = "\n".join(outputs)
            try:
                result = subprocess.run(['python', temp_program_path], input=inputs, text=True, capture_output=True, timeout=timeout, preexec_fn=_set_memory_limit)
                exec_code = 999
            ...
            if exec_code > 0:
                if result.returncode != 0:
                    try:
                        inputs_tmp_path = _write_temp_file(inputs, tmpdir, f"input_{i}.txt")
                        with open(inputs_tmp_path, 'r') as inputs_tmp_file:
                            result = subprocess.run(['python', temp_program_path], stdin=inputs_tmp_file, text=True, capture_output=True, timeout=timeout, preexec_fn=_set_memory_limit)
                        ...
                        try:
                            inputs_file = os.path.join(tmpdir, 'input.txt')
                            output_file = os.path.join(tmpdir, 'output.txt')
                            with open(inputs_file, 'w') as ftemp:
                                ftemp.write(inputs)
                            result = subprocess.run(['python', temp_program_path], text=True, timeout=timeout, cwd=tmpdir, preexec_fn=_set_memory_limit)
                            assert result.returncode == 0
                            if compare_std_results(open(output_file).read(), outputs, debug):
                                exec_code = 1
                            else:
                                exec_code = 0
                        except:
...
def _write_temp_file(content, directory, filename):
    """Write content to a named file inside the given directory."""
...
def reliability_guard(maximum_memory_bytes=None):
    if maximum_memory_bytes is not None:
        ...
        for limit in (resource.RLIMIT_DATA, resource.RLIMIT_AS):
            try:
                resource.setrlimit(limit, (maximum_memory_bytes, maximum_memory_bytes))
            except (ValueError, OSError):
                pass
